...
func main() {
	...
	catalog := flag.String("catalog", "canboatjson-v8.1.0.cache", "Canboat JSON catalog")
	flag.Parse()
	...
	for _, file := range files {
		inspectFile(file, fields)
	}
}

func loadLAUFields(path string) (map[uint32][]string, error) {
	f, err := os.Open(path) //nolint:gosec // The user explicitly supplies the catalog path.
	if err != nil {
		return nil, err
	}
	...
	for _, def := range catalog.PGNs {
		for _, field := range def.Fields {
			if field.Type == "STRING_LAU" {
				result[def.PGN] = append(result[def.PGN], field.Name)
			}
		}
	}
	return result, nil
...
func inspectFile(path string, fields map[uint32][]string) {
	f, err := os.Open(path) //nolint:gosec // Paths come from the user-selected replay directory.
	if err != nil {
		logrus.WithError(err).Warn(path)
		return
	}
	defer f.Close() //nolint:errcheck // Read-only input.
	...
	for {
		...
		pgnValue, err := strconv.ParseUint(record[2], 10, 32)
		if err != nil {
			continue
		}
		fieldNames, ok := fields[uint32(pgnValue)]
		if !ok {
			continue
		}
		source, err := strconv.ParseUint(record[3], 10, 8)
		if err != nil {
			continue
		}
		data := parseBytes(record[6:])
		if len(data) == 0 {
			continue
		}
		packet := &pkt.Packet{Info: pgn.MessageInfo{PGN: uint32(pgnValue), SourceId: uint8(source)}, Data: data}
		if len(data) > 8 {
			report(path, record[0], packet, fieldNames)
			continue
		}
		builder.Add(packet)
		if packet.Complete {
...
func parseBytes(values []string) []byte {
	...
	for _, value := range values {
		b, err := strconv.ParseUint(strings.TrimSpace(value), 16, 8)
		if err == nil {
...
func report(file, timestamp string, packet *pkt.Packet, names []string) {
	for offset := 0; offset+2 <= len(packet.Data); offset++ {
		...
		if length < 3 || encoding > 1 || offset+length > len(packet.Data) {
			continue
		}
		...
		fmt.Printf(
			"file=%s time=%s pgn=%d source=%d fields=%q offset=%d length=%d "+
				"encoding=%d terminated=%t value=%q raw=% x\n",
			filepath.Base(file), timestamp, packet.Info.PGN, packet.Info.SourceId,
			names, offset, length, encoding, terminated, string(value), packet.Data[offset:offset+length],
		)
	}
}
