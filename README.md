...
The dumpcan command connects to a CAN interface and dumps all NMEA 2000 PGNs to standard output in a human-readable format. It's useful for monitoring and debugging NMEA 2000 network traffic in real-time.

Usage: `dumpcan -iface <interface_name>`

The command will run until terminated with Ctrl+C or SIGTERM.

**Note**: Virtual CAN interfaces (vcan) are supported through the SocketCAN endpoint. See [docs/testing-alternatives.md](docs/testing-alternatives.md) for details.
